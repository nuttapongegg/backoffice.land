$(document).ready(function () {
  ...
  $formEditRealInvestment
    ...
    .on("click", ".btnSaveRealInvestment", function (e) {
      ...
      formData.append(
        "content",
        $formEditRealInvestment.find(".ql-editor").html()
      );
  ...
  $formAddLandAccount
    ...
    .on("click", ".btnSaveLandAccount", function (e) {
      ...
      // เช็คข้อมูล
      if (
        $formAddLandAccount.find("input[name=land_account_name]").val() == ""
      ) {
        alert("กรุณาระบุชื่อบัญชี");
        return false;
      }
      if (
        $formAddLandAccount.find("input[name=land_account_cash]").val() == ""
      ) {
      ...
      // ผ่าน
        ...
        formData.append(
          "content",
          $formAddLandAccount.find(".ql-editor").html()
        );
  ...
  $("body").on("click", ".btnEditLandAccount", function () {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        $("#updateLandAccount #edit_land_account_name").val(
          res.data.land_account_name
        );
        $("#updateLandAccount #edit_land_account_cash").val(
          res.data.land_account_cash
        );
      },
      error: function (data) {},
    });
  ...
  $formEditLandAccount
    ...
    .on("click", ".btnEditAccount", function (e) {
      ...
      // เช็คข้อมูล
      if (
        $formEditLandAccount.find("input[name=edit_land_account_name]").val() ==
        ""
      ) {
        alert("กรุณาระบุชื่อบัญชี");
        return false;
      ...
      if (
        $formEditLandAccount.find("input[name=edit_land_account_cash]").val() ==
        ""
      ) {
        alert("กรุณาระบุจำนวนเงิน");
        return false;
      ...
      // ผ่าน
        ...
        formData.append(
          "content",
          $formEditLandAccount.find(".ql-editor").html()
        );
  ...
  $formAddLandAccountPlus
    ...
    .on("click", ".btnSaveLandAccountPlus", function (e) {
      ...
      if (
        $formAddLandAccountPlus
          .find("input[name=land_account_money_plus]")
          .val() == ""
      ) {
        alert("กรุณาระบุจำนวนเงิน");
        return false;
      ...
      // ผ่าน
        ...
        formData.append(
          "content",
          $formAddLandAccountPlus.find(".ql-editor").html()
        );
  ...
  $formAddLandAccountMinus
    ...
    .on("click", ".btnSaveLandAccountMinus", function (e) {
      ...
      if (
        $formAddLandAccountMinus
          .find("input[name=land_account_money_minus]")
          .val() == ""
      ) {
        alert("กรุณาระบุจำนวนเงิน");
        return false;
      ...
      // ผ่าน
        ...
        formData.append(
          "content",
          $formAddLandAccountMinus.find(".ql-editor").html()
        );
  ...
  $formlTransferLandAccount
    ...
    .on("click", ".btnSaveTransferLandAccount", function (e) {
      ...
      if (
        ...
        $formlTransferLandAccount
          .find("input[name=transfer_money_land_account]")
          .val() == ""
      ) {
        alert("กรุณาระบุจำนวนเงิน");
        return false;
      ...
      // ผ่าน
        ...
        formData.append(
          "content",
          $formlTransferLandAccount.find(".ql-editor").html()
        );
  ...
  $("body").on("click", ".btnTransferLandAccount", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        var selectElement = $(
          '#TransferLandAccount select[name="land_account_name"]'
        );
        ...
        LandAccounts.forEach(function (LandAccount) {
          ...
          if (LandAccountId !== parseInt(LandAccount_id)) {
            ...
            var isSelected =
              LandAccountId === parseInt(LandAccount_id) ? "selected" : "";
            var option =
              '<option value="' +
              LandAccount.id +
              '" ' +
...
document.addEventListener("DOMContentLoaded", function () {
  const selectAction = document.getElementById("unitEditRealInvestment");
  const editAmountGroup = document.getElementById("editAmountGroup");
  const realInvestmentInput = document.getElementById("realInvestment");
  const editInvestmentInput = document.getElementById("editRealInvestment");
  ...
  function toggleEditAmount() {
    const value = selectAction.value;
    if (value === "เพิ่ม" || value === "ลบ") {
      editAmountGroup.style.display = "block";
    } else {
      editAmountGroup.style.display = "none";
      calculatedAmount.textContent = ""; // ซ่อนผลลัพธ์เมื่อไม่ใช่ เพิ่ม/ลบ
    }
    calculateTotal(); // คำนวณทันทีเมื่อเปลี่ยนประเภท
  }

  function calculateTotal() {
    const action = selectAction.value;
    const base = parseFloat(realInvestmentInput.value.replace(/,/g, "")) || 0;
    const amount = parseFloat(editInvestmentInput.value.replace(/,/g, "")) || 0;

    let result = 0;
    if (action === "เพิ่ม") {
      result = base + amount;
      calculatedAmount.textContent = ` ( ${formatNumber(result)} บาท)`;
    } else if (action === "ลบ") {
      result = base - amount;
      calculatedAmount.textContent = ` ( ${formatNumber(result)} บาท)`;
    } else {
  ...
  // Event listeners
  selectAction.addEventListener("change", toggleEditAmount);
  editInvestmentInput.addEventListener("input", calculateTotal);
});
